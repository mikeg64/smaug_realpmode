...
%% perturbed and background b field   
%       val3=reshape((wd(6,nr1,nr2,nr3)+wd(11,nr1,nr2,nr3)),snr1(2),snr2(2),snr3(2));
%    uu=shiftdim(val3,1);
%    
%         val3=reshape((wd(7,nr1,nr2,nr3)+wd(12,nr1,nr2,nr3)),snr1(2),snr2(2),snr3(2));
%    uv=shiftdim(val3,1);
%    
%         val3=reshape((wd(8,nr1,nr2,nr3)+wd(13,nr1,nr2,nr3)),snr1(2),snr2(2),snr3(2));
%    uw=shiftdim(val3,1);

%% pertubed b-field only  
...
figure;
  hold on;
         ...
         quiver3(x1,x2,x3,1000.*uu,1000.*uv,1000.*uw,'Color',[0 0.498 0], 'LineWidth',1.5)
       
       hold on;
     %h=slice(myval,65, 65,[5 49 100]); 
     %h=slice(myval,[], [63],[5 49 100]); 
%       h=slice(myval,[], [61],[5 49 ]);  
%        
%        
%        hold on;
...
% TP=shiftdim(mu_gas.*TP./R./val2,1);
   ...
   maxv=max(max(max(bmag)));
minv=min(min(min(bmag)));
  %isovalue=0.1909;
...
%    [x1,x2,x3] = meshgrid(nrange1,nrange2,nrange3);
   ...
   [x1,x2,x3] = meshgrid(nrange2,nrange3,nrange1);
%   x1=shiftdim(x1,1);
%     x2=shiftdim(x2,1);
%     x3=shiftdim(x3,1);
    ...
    % isovalue1=2.0e6;
    isovalue1=0.02;
     fv1 = patch(isosurface(x1,x2,x3,bmag,isovalue1));
      isonormals(x1,x2,x3,bmag,fv1);
