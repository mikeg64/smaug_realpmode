...
%directory='/fastdata/cs1mkg/smaug/spic_4b2_2_bv20G/';
...
directory1='/Users/mikegriffiths/proj/smaug-realpmode-data/0g/';
...
filename1=[directory1,'zerospic1__',id,extension];
...
disp([id filename]);
   ...
   for iw=1:nw
      ...
      fid1=fopen(trim(filename1));
   %fseek(fid,pictsize(ifile)*(npict(ifile)-1),'bof');
   headline=trim(setstr(fread(fid1,79,'char')'));
   it=fread(fid1,1,'integer*4'); time=fread(fid1,1,'float64');
 
   ndim=fread(fid1,1,'integer*4');
   neqpar=fread(fid1,1,'integer*4'); 
   nw=fread(fid1,1,'integer*4');
   nx=fread(fid1,3,'integer*4');
   
   nxs=nx(1)*nx(2)*nx(3);
   varbuf=fread(fid1,7,'float64');
   ...
   varnames=trim(setstr(fread(fid1,79,'char')'));
   
   for idim=1:ndim
      X(:,idim)=fread(fid1,nxs,'float64');
   end
   
   for iw=1:nw
      %fread(fid,4);
      w1(:,iw)=fread(fid1,nxs,'float64');
      %fread(fid,4);
   end
...
%plot sections through 3d array
   ...
   [x1,x2,x3] = meshgrid(ax,ay,az);
     val1=reshape(wd(2,nrange,nrange,nrange),124,124,124);%z component
      val1_1=reshape(wd1(2,nrange,nrange,nrange),124,124,124);%z component
%    val1=reshape(wd(3,nrange,nrange,nrange),124,124,124);
   ...
   val2_1=reshape(wd1(1,nrange,nrange,nrange)+wd1(10,nrange,nrange,nrange),124,124,124);

   val1=(val1./val2);
   val1_1=(val1_1./val2_1);
   val3=(val3./val2);

 %  val4=sqrt(val1.^2 + val3.^2);

   
    myval=(shiftdim(val1_1,1)-shiftdim(val1,1))./(shiftdim(val1_1,1)+shiftdim(val1,1));
...
% end
  ...
  %set(hc,'Ylim',[4*10^5 3*10^6]);
   text(-100,0,175,timetext, 'FontSize', 18);
