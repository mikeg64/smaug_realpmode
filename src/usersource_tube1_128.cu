   ...
   qt=p->qt;
    ...
    tdep=sin(qt*2.0*PI/s_period);
        ...
        vz=aa*exp_xyz*tdep; 
                           ...
                           w[fencode3_MODID(p,ii,mom1)]+=(p->dt)*vz*(w[fencode3_MODID(p,ii,rho)]+w[fencode3_MODID(p,ii,rhob)]);
                          ...
                          w[fencode3_MODID(p,ii,energy)]+=(p->dt)*(vx*vx+vy*vy+vz*vz)*(w[fencode3_MODID(p,ii,rho)]+w[fencode3_MODID(p,ii,rhob)])/2.0;
