...
\Title{{$p$}%MDPI:Please use a consistent style for the letters throughout the paper, (e.g., italic or not).
-Mode Oscillations in Highly Gravitationally Stratified Magnetic Solar Atmospheres}

% MDPI internal command: Title for citation in the left column
\TitleCitation{\emph{{p}}-Mode Oscillations in Highly Gravitationally Stratified Magnetic Solar Atmospheres}
...
}$, Ruisheng Zheng $^{3}$, Marianna Kors\'os $^{1,4,5,6}$\orcidB{} \linebreak and Robertus Erd\'{e}lyi $^{1,5,6}$\orcidC{}}


%\longauthorlist{yes}

% MDPI internal command: Authors, for metadata in PDF
\AuthorNames{Michael Griffiths, Norbert Gyenge, Ruisheng Zheng, Marianna Kors??s and Robertus Erd\'{e}lyi}

% MDPI internal command: Authors, for citation in the left column
\AuthorCitation{Griffiths, M.; Gyenge, N.R.; Zheng, R.; Kors??s, M; Erd\'{e}lyi, R.}
...
95123~Catania,~Italy\\
$^{5}$ \quad Deptartment of Astronomy, E\"otv\"os L. University, P\'azm\'any P\'eter s\'et\'any 1/a, %\hl{H-}
 1117 Budapest, Hungary\\
$^{6}$ \quad Solar Physics and Space Plasma Research Centre (SP$^{2}$RC), School of Mathematics and Statistics, University~of~Sheffield, Hicks Building, Hounsfield Road, {Sheffield S3 7RH}%MDPI: We added city and revised post code by google, please confirm. MKG Confirmed
, UK
}
...
\abstract{The aim of the 
 %work 
{study} %MDPI: "study", "paper", "investigation", "revoiiew" instead of "work", "article", "manuscript" etc. recommended for scientific papers. Please consider. Here and elsewhere below.
reported in this paper is to gain understanding of solar global oscillations and the propagation characteristics of $p$-mode oscillations in the highly gravitationally stratified magnetic solar atmosphere. The paper presents the results of 
 {3D (3-dimensional)} %MDPI: No undefined abbreviations, variables, notations, indexes ets. when first met in abstract, maintext, figures, tables. 
...
In order to develop an improved model providing a representation of the solar atmosphere, it is necessary to establish that the modelling tools give a consistent behaviour in idealised test cases and that there is a consistency between the computational and theoretical models.  Following this principle, an~earlier 
 %work 
 {study}  
tested the SMAUG {(Sheffield MHD accelerated using graphical processing units)} %MDPI: defined as  met first in the main text. 
...
Sun occurred for a range of frequencies. Such observations may explain the observed intensity oscillations for periods greater than the 
 %well-
{observed} %MDPI: "well/widely/etc-known/used/etc" not recommended for scientific papers. Please consider. MKG changed known to observed
...
The %~Sheffield MHD Accelerated Using GPUs (SMAUG) 
 {SMAUG} %MDPI: already defined above. 
...
general system of ideal MHD equations applicable to an ideal compressible plasma, %Please check intended meaning is retained
 ...
 {the} 
initial hydrodynamic {simulations~\cite{Griffiths2018b},} %MDPI: punctuations corected. here and through the paper. 
 %.
\begin{eqnarray}
...
\noindent
{The } %MDPI: Please confirm the indent of the first line. If it is a paragraph, we suggest: Indentation: Left: 4.6 cm; First line: 0.75 cm.
...
is the gravitational acceleration vector, $\gamma$ is the adiabatic parameter, $t$ is the time,  %MDPI: please complete, confirm. MKG added adiabatic parameter... 
and  
...
{Next, in Section} \ref{sec:structures}, %4, %MDPI: always useful to point teh section, equation numbers to better orien the reader.  
...
%Solar active regions contain sunspots which have sizes between  1 - 50 Mm. 
 ...
 %Active Regions 
 {active regions} %MDPI: used this way all over the paper. Suggested the lower-case always. Please consider.
...
%many north poles in the quiet photosphere, this is known as 
 ...
 %it 
  {Figure}~\ref{fig1} %MDPI: makes it clearer.
...
\caption{{A schematic} %MDPI: Figures should be placed after their first mention in the paper. Therefore, we have changed the placement for some of the Figures in the manuscript. Please confirm (across the section titles, espessially the first-level sections). MKG confirmed
 representation of the solar magnetic network. 
 {See text for details.} %MDPI: Otherwise, needs to be described. 
...
\caption{\hl{The} %MDPI: 1. Please cite the figure in the text and ensure the first citation of each figure appears in numerical order.
 ...
 {that}%MDPI: The bold is removed, looks unnecessary.  The following highlights are the same.
...
\cite{Griffiths2018b} employed simulation drivers with physical characteristics representing $p$-mode oscillations with varying modes and periods. The~MHD simulations reported here use the same driver and model of the solar atmosphere as used in our initial hydrodynamic study. The~model is  advanced by the inclusion of a uniform, vertical and cylindrically symmetric magnetic~field. 
 ...
 {$128 \times128 \times128 $.} %MDPI: any units? MKG dimensionless number so no units  
...
\caption{{Atmospheric} %MDPI: Please change the hyphen (-) into a minus sign ($-$, "U+2212"). e.g., "-1" should be "$-$1".
 profiles for the density and temperature for the model solar atmosphere based on the 
VALIIIc~model~{\cite{Vernazza1981}.} %MDPI: helps the reader.
\label{fig3}}
\end{figure}
\unskip
...
The simulations, presented in this paper, employ an extended driver resulting in the perturbation of the entire lower boundary of the model.  The~photospheric $p$-mode oscillations  that are observed on the Sun have a horizontal wavelength and coherence. The~vertical velocity driver used here is an acoustic $p$-mode driver located at the photosphere and exciting waves which propagate into our realistic 3D model geometry of the solar atmosphere. An~extended driver with a sinusoidal dependence and a wavelength of 8~Mm applied along the middle of the base of a computational domain of dimension 4 Mm represents  a 
 {{\it fundamental mode}} %MDPI: is italics necessary? Reserved for subtitles.   
component of the global acoustic oscillations. Drivers may be constructed as an ensemble of these solar global eigenmodes. The~driver is approximated by the 
 {expression,} %MDPI: clear enough. Please consider.  
...
\end{adjustwidth}
\caption{{The vertical} %MDPI: 1. Please change the hyphen (-) into a minus sign ($-$, "U+2212"). e.g., "-1" should be "$-$1".
...
Sheffield %, \citet{Griffiths2018a} 
 {\cite{Griffiths2018a}}. %MDPI: or "by Griffiths, ... " adding all three authors. 
...
A  time--distance plot for the 300~s period driver, with~the 
 {100~G} %MDPI: looks Figure 8 to come before Figure 7, the latter shows the 0 G case. Please see just below and correct accordingly. 
field is shown in {Figure~\ref{fig7}}%MDPI: Figures should be numbered in order of appearance. We detected "Figure 8" appears before "Figure 7", please rearrange all the references to appear in numerical order. MKG numerical references corrected as requested
...
\caption{{Time--distance} %MDPI: 1. Please change the hyphen (-) into a minus sign ($-$, "U+2212"). e.g., "-1" should be "$-$1".
 ...
 plot of the vertical component of the velocity in the mid-chromosphere, for~the magnetic field of 100~{G. %, the
...
\caption{{The %table shows wave %MDPI: 1. clear enough. 2. No secondary \midrule necessary.
...
expression for the energy flux which was used by~\cite{Bogdan2003}. The~wave energy flux 
 {${\bf F}_{\rm wave}$} %MDPI: No bold for "wave" seems necessary, so removed.
 is given by
$$
...
%. %MDPI: Moved here as met first.

We compute the {time-averaged} %MDPI: the dash clarifies. Unclear otherwise. 
...
cm]{energyfluxratio.jpg}
    \caption{{The ratio} %MDPI: 1. Please change the hyphen (-) into a minus sign ($-$, "U+2212"). e.g., "-1" should be "$-$1".
 ...
 {0~G (blue),} 50~G (orange), 75~G (purple), and 100~G (red). %MDPI: first the values as the sentence says it.
\label{fig9}}
\end{figure}
\unskip
...
Figure~\ref{fig10} {(top)} %MDPI: no "panel". Here and elsewhere below. 
shows a vertical slice of %$Bz$
 {$B_z$} %MDPI: corrected. Please confirm. 
...
A %n~
 {fast Fourier transform %MDPI: No undefined abbreviations even known or listed in the Abbreviations table.
(FFT)} was applied ({see} %the lower panel of 
...
Figure~\ref{fig11} features the same analysis as Figure~\ref{fig10}; however, the~study features a vertical slice of 
 %$Vz$ 
 {$V_z$} %MDPI: please confirm. MKG confirmed
...
.5cm]{fft_sim.pdf}
    \caption{{Temporal} %MDPI: the size of the figure increased to make it better visible. Please confirm. Same for Fig. 11. MKG confirmed  
analysis of $B_{z}$ vertical slices at $2$ {Mm:} {({\bf top})} %. The~top panel shows 
the selected vertical slices, indicated by gray 
 {colours;  ({\bf middle})} %. The~middle panel demonstrates 
the obtained signal after applying a Hanning window {function; and ({\bf bottom})} %. The~bottom panel shows 
...
\begin{figure}[H]
   % \centering
    \includegraphics[width=9%8
.5cm]{fft_sim2.pdf}
    \caption{Temporal analysis of $V_{z}$ vertical slices at $2$~{Mm:} {({\bf top})} %.  The~top panel shows 
 the selected vertical slices, indicated by gray {colours;  ({\bf middle})} %. The~middle panel demonstrates 
 the obtained signal after applying a Hanning 
window {function; and ({\bf bottom})} %. The~bottom panel shows 
...
With the objective of confirming the obtained oscillatory behaviour, temporal analysis for the observational 
 {data} \highlighting{\cite{Pesnell2012,Lemen2012}} %MDPI: please add the reference. MKG done 
was performed. We investigated intensity oscillations in the solar atmosphere observed by 
SDO/AIA  {(Atmospheric Imaging Assembly)} \highlighting{\cite{Lemen2012}.}%MDPI: data ref. added. Please confirm. 
...
also normalised by applying standard scores ({$Z$}-scores), %MDPI: math mode, please confirm.
defined by 
\begin{equation}
...
Figure~\ref{fig12} %MDPI: clear enough from the captions and axes labels. 
 {demonstrates} the trend removed and normalised time series {($Z$-scores)} %MDPI: clarifies.
%. The~lower panel of {Figure~\ref{fig11}  
 {and shows} %MDPI: clear enough from the captions and axes labels. 
...
$\sigma(N_{b})$,} 
and the 
 {mean, $\overline{N_{b}}$,} %MDPI: the commas added as the definiions introduced. 
...
%MDPI: please add the reference. 
between 18:00 UT to 20:00 UT on 22 August 2010 \highlighting{\cite{Lemen2012}.}%MDPI: data ref. added. Please confirm.. 
 {The} %~upper panels show the temporal
 variation of the {$Z$-score} 
(de-trended by first difference and normalised pixel intensity data) {and} %. The~lower panel shows 
the FFT of the 
analysed observational~data {are shown as indicated.} 
 {The dashed line} indicates  the significance level of 3 standard deviation calculated using a Monte Carlo method. %MDPI: added, otherwise unclear. Please consider. MKG OK
\label{fig12}}
\end{figure}
...
%(Hz) %MDPI: clear from teh Table.  
 ...
 %Quiet
 {quiet}  
Sun between 18:00 UT to 20:00 UT on 22 August {2010 \highlighting{\cite{Lemen2012}},} %MDPI: Data ref. added; pleaase confirm.
...
The  {review} %of~ %MDPI: clear enough.
...
An initial study of global oscillations in a highly gravitationally stratified atmosphere looked at the variation of energy leakage with different modes of oscillation of an extended driver~\cite{Griffiths2018b}. The~investigation addressed an observational study indicating the ubiquitous nature of intensity oscillations. The~power spectra presented in 
 %the first figure 
 {Figure~1} %MDPI: please confirm the meaning retained. MKG confirmed thank you 
of~{Ref.} 
...
There is an issue that due to the extended nature of the driver, the~amplitudes are responsible for delivering vast quantities of energy into the solar atmosphere inducing extremely large shocks and result in a numerically unstable system~\cite{Santamaria2015}. Significant improvements to the comparisons presented in this paper are expected with the  recent 
 %work 
 {study} of {Ref.}~\cite{Kostogryz2021}. Here, using the ADIPLS {code} %MDPI: clear enough.
%of
...
in~{Ref.} 
\highlighting{\cite{Rast2016}}%MDPI: Refs. [59], [60], and [61] are missing their citation. Please add and rearrange all the references to appear in numerical order.
...
\hl %MDPI: any preference of this order and not in the order of the author list? If no preference, please rearrange due to the author list here and below.
M.G.,  N.G.,R.Z., {M.K.} %MDPI: We revised M.B.K. to M.B., please confirm. MKG you mean to MK
 and {R.E.,}; methodology, 
M.G., N.G. and \hl{R.E.} ; software, M.G. and N.G.; validation, M.G. and N.G.; formal analysis, 
 M.G., N.G. and \hl{R.E.}; writing---original draft preparation, M.G.; writing---review and editing, 
\hl M.G., N.G., M.K. and {R.E.};  
...
739500 (PRE-EST project) and no. 824135 (SOLARNET project).
 {M.K.}
is grateful to ST/S000518/1, PIA.CE.RI. 2020-2022 Linea2, CESAR 2020-35-HH.0, and~UNKP-22-4-II-ELTE-186 grants (UK).} %MDPI: please confirm or indicate countries for each if different. MKG confirmed


\dataavailability{{The data used can be obtained in the references cited and on request from the corresponding author.}} %MDPI: please confirm.
...
\acknowledgments{The authors thank P.H. Keys for providing the wavelet tools to analyse the related SDO {data.} %MDPI: the grant parts moved to the Funding section. Please consider. 
...
R.E. and 
 {M.K.}  
are also grateful to {NKFIH}, National Research and Innovation Office 
(OTKA, {the Hungarian Scientific Research Fund}, grant No. K142987), Hungary,  for enabling this research.
We acknowledge IT Services at The 
University of Sheffield for the provision of the 
 %High Performance Computing~Service.
 \hl{high performance computing~service.} %MDPI: are the capital letters necessary? 
}
...
solar model. \aap~\textbf{2007}, \emph{466}, 377--388. https://doi.org/10.1051/0004-6361:20041632.
		...
		\bibitem[Mein and Mein(1976)]{Mein1976}Mein, N.; Mein, P.  {Velocity waves in the quiet solar chromosphere.}%MDPI: Newly added information by google, please confirm, same as refs 42,48,59.
		 \emph{Sol. Phys.} \textbf{1976}, \emph{49}, 231{--248.} https://doi.org/10.1007/BF00162447.
		
		\bibitem[Schmieder \& Mein(1980)]{Schmieder1980} Schmieder, B.; Mein, N. {Mechanical flux in the solar chromosphere. II. 
...
{Available online:} \url{https://ui.adsabs.harvard.edu/abs/1980A%26A....84...99S/} (accessed on 18 March 2023).
		...
		\bibitem[De Moortel (2009)]{deMoortel2009} 
...
seismology}. \ssr~\textbf{2011}, \emph{158}, 397{--419.}  https://doi.org/10.1007/s11214-010-9716-1.
		...
		\bibitem[Auch{\`e}re et al. (2014)]{Auchere2014} Auch??re, F.; Bocchialini, K.; Solomon, J.; Tison, E. Long-period intensity pulsations in the solar corona during activity cycle 23. \aap~\textbf{2014}, \emph{563}, A8. https://doi.org/10.1051/0004-6361/201322572.
...
appearances. \apj~\textbf{2018}, \emph{859}, 169. https://doi.org/10.3847/1538-4357/aac109.
		...
		\bibitem[{Gudiksen} et~al.(2011){Gudiksen}, {Carlsson}, {Hansteen}, {Hayek}, {Leenaarts}, and {Mart{\'{\i}}nez-Sykora}]{Gudiksen2011}		Gudiksen, B.V.; Carlsson, M.; Hansteen, V.H.; Hayek, W.; Leenaarts, J.; Mart??nez-Sykora, J. The stellar atmosphere simulation codeBifrost. \aap~\textbf{2011}, \emph{531}, A154. https://doi.org/10.1051/0004-6361/201116520.
...
magnetized solar atmosphere}. \apj~\textbf{2012}, \emph{755}, {A18.} %MDPI: article's OID. %1--
...
SMAUG. \emph{J. Astrophys. Astron.} \textbf{2015}, \emph{36}, 197--223. https://doi.org/10.1007/s12036-015-9328-y.
 ...
 {Available online:} \url{https://ui.adsabs.harvard.edu/abs/1996ApL%26C..34..245T} (accessed on 18 March 2023). 
		...
		\bibitem[Cameron \emph{et al.}(2007)]{Cameron2007}Cameron, R.; Sch??ssler, M.; V??gler, A.; Zakharov, V. Radiative magnetohydrodynamic simulations of solar pores. \aap~\textbf{2007}, \emph{474}, 261--272. https://doi.org/10.1051/0004-6361:20078140.
...
equilibrium---I. Three-dimensional open magnetic flux tube in the stratified solar atmosphere. \emph{Mon. Not. R. Astron. Soc.} \textbf{2013}, \emph{435}, 689--697. https://doi.org/10.1093/mnras/stt1328.
		...
		\bibitem[Sch{\"u}ssler and Rempel(2005)]{Schussler2005}Sch??ssler, M.; Rempel, M. The dynamical disconnection of sunspots from their magnetic roots. \aap~\textbf{2005}, \emph{441}, 337--346. https://doi.org/10.1051/0004-6361:20052962.
		
		\bibitem[{{Griffiths} et~al.(2018a){Griffiths2018a}, {Erd{\'e}lyi}}]{Griffiths2018a}
		{Griffiths}, M.; 
  {von Fay-Siebenburgen, R.;} %MDPI; we chnaged the list due to the names given on webpage. Please check.   
{Erd{\'e}lyi}. %, R.; {Fedun}, V. 
{Videos of p-Mode Oscillations in Highly Gravitationally Stratified Magnetic Solar Atmospheres}. 
 {The University of Sheffield,} %MDPI; we added the location. Please confirm.  
...
S.W.; Nordlund, P. Waves in the magnetized solar atmosphere. II. Waves from localized sources in magnetic flux concentrations. \apj~\textbf{2003}, \emph{599}, 626--660. https://doi.org/10.1086/378512.
  ...
  %SDO
\bibitem[Pesnell et al.(2012)]{Pesnell2012} \hl{Pesnell,} %MDPI: newly added/corrected information, ref. list reordered. Please confirm. 
...
%AIA
\bibitem[Lemen et al.(2012)]{Lemen2012} Lemen, J.R., Title, A.M., Akin, D.J.,  
\hl{Boerner, P.F.;} %MDPI: up to first 10 authors to be listed. 
Chou, C.; 
Drake, J.F.;  
Duncan, D.W.;  
Edwards, C.G.;  ·
...
https://doi.org/10.1007/s11207-011-9776-8
 ...
 %---Part One---
 {I:} %MDPI: corrected due to teh publication. please check.
